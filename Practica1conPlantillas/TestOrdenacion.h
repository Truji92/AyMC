...
public:
    ...
    Ordenacion<T> *estrategia; // puntero al tipo de ordenacion a realizar
};

template<class T>
class TestOrdenacion {
    vector<tOrdenacion<T> > testMetodos; // contiene todos los métodos a estudiar
    int num_pruebas_al_comparar;

    
    double ordenarArray(T v[],int size,int metodo);
public:
    /**
     ...
     */
    vector<tOrdenacion<T> > getTestMetodos() {
        return testMetodos;
    }
