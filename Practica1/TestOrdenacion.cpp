...
void pause() {
    ...
    TestOrdenacion::TestOrdenacion() {
        ...
        FicherosYGraficas::setTest(this);
}
...
void TestOrdenacion::compararMetodos(vector<int> met) {
    ...
    FicherosYGraficas::GuardarEnArchivo(array_tiempos, (tallaFin - tallaIni) / incTalla + 1, met);
    pause();
...
void TestOrdenacion::estudiarMetodo(int m) {
    ...
    FicherosYGraficas::GuardarEnArchivo(array_tiempos, (tallaFin - tallaIni) / incTalla + 1, m);
    pause();
}
